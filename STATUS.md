...
* 참고 링크 다양화는 많이 개선됐지만, 여러 규칙이 동시에 적용되는 음식은 서로 다른 rule에서 같은 병원 링크가 한 번 더 보일 수 있어 후속 dedupe 정리가 여전히 가능하다.
...
* SearchLog 상위 fallback 검색어를 다시 보고 3차 alias / food 후보를 대량 보강하기
* direct food source를 더 붙여 개별 음식에서도 병원 상세 URL이 더 직접적으로 보이게 만들기
* 너무 넓게 확장된 음식군 중 판정이 과한 항목이 없는지 representative QA를 더 넓게 돌리기
* 병원 안내문에 직접 등장하는 음식 예시를 더 수집해 `food direct source` 비중을 높이기
* 참고 근거 링크가 rule 단위 중복 없이 더 깔끔하게 보이도록 전체 dedupe 한 번 더 정리하기
