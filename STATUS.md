...
* 단계: Phase 19~21 완료, prewarm/SEO 기본 적용 완료, 공유/FAQ/전날 허용식 UX 보정 완료, 결과 상세/FAQ 중복 축소와 감자 전날 허용식 보정 완료, 대량 DB 확장 1차 및 production 반영 완료
* 진행률: MVP 100%, Launch Prep 97%
* 마지막 업데이트: 2026-04-18
...
* 공식 병원 안내문 공통 기준을 바탕으로 bulk catalog를 새로 추가해 `food 423개 / alias 87개 / foodGroup 37개 / source 13개`까지 정적 데이터를 대폭 확장
...
* 병원 안내문이 개별 음식명을 모두 직접 나열하는 것은 아니어서, 이번 bulk food 상당수는 `공식 병원 공통 식이 기준(건더기·매운 양념·튀김·가공·해조류·잡곡 등)`에 맞춰 그룹화한 보강 데이터다.
* 참고 링크 다양화는 많이 개선됐지만, 여러 규칙이 동시에 적용되는 음식은 서로 다른 rule에서 같은 병원 링크가 한 번 더 보일 수 있어 후속 dedupe 정리가 여전히 가능하다.
* alias 수는 `87개`까지 늘었지만, 대표 음식 수 증가폭에 비해 실제 검색어 별칭은 아직 더 보강할 여지가 크다.
...
* SearchLog 상위 fallback 검색어를 다시 보고 2차 alias / food 후보를 대량 보강하기
