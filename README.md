...
로컬에 PostgreSQL이 없다면 Docker로 바로 띄울 수 있습니다.
...
curl -sG "http://127.0.0.1:3000/api/check" \
  ...
  --data-urlencode "query=국수"

curl -sG "http://127.0.0.1:3000/api/check" \
  --data-urlencode "condition=colonoscopy" \
  --data-urlencode "dayStage=d3" \
  --data-urlencode "query=초코도넛"
```

### 7) SearchLog 확인

Docker PostgreSQL 기준 예시:

```bash
docker exec health-signal-postgres psql -U postgres -d health_signal_dev -c \
  "select query, \"matchedType\", \"resultStatus\", \"confidenceGrade\" from \"SearchLog\" order by \"createdAt\" desc limit 10;"
```
...
- 현재 seed는 초기화 성격이 있으므로 production에서는 최초 1회 기준으로만 사용한다.
- 운영 중 데이터 보강은 전체 seed 재실행이 아니라 필요한 `alias`, `food`, `rule`만 점진 반영한다.

자세한 운영 기준은 [docs/deployment-ops.md](/Users/badal/dev/projects/colono-food-check/docs/deployment-ops.md), [docs/production-runbook.md](/Users/badal/dev/projects/colono-food-check/docs/production-runbook.md), [docs/operations-loop.md](/Users/badal/dev/projects/colono-food-check/docs/operations-loop.md), [docs/fallback-triage.md](/Users/badal/dev/projects/colono-food-check/docs/fallback-triage.md) 에 정리했다.

브라우저에서 `http://localhost:3000` 으로 접속하면 기본 검색 UI를 사용할 수 있습니다.
...
대표 음식 20개에 대한 판정 엔진 기대값과 `/api/check` 기본 에러 응답을 검증합니다.
