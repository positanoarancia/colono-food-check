...
* [x] 상단 신뢰 문구를 `국내 주요 대학병원 안내문 기반 + 병원 안내 우선` 톤으로 다듬고, 두 줄 간격을 더 촘촘하게 보정
* [x] 결과 카드에 `출처: 병원명`을 바로 노출하고, fallback에서도 공통 병원 가이드 링크가 보이도록 신뢰도 표현 보강
* [x] 상태 라벨을 `먹어도 괜찮아요 / 가급적 먹지 마세요 / 절대 먹지 마세요` 톤으로 조정
* [x] `caution / avoid / fallback` 결과에서 안전한 대체 음식이 최소 1개 이상 보이도록 보강
* [x] `회` 같은 검색어도 fallback 카테고리 추론으로 더 단정적인 안내가 나오도록 보강
...
* [x] 핵심 direct food source coverage를 보강하고 최소 source coverage smoke test 추가
...
* [x] 한 글자 입력은 음식 판정 대신 입력 보정형 fallback으로 분리해 과한 경고를 줄임
* [x] 결과 상세 토글과 FAQ 사이 밀도를 더 낮춘 프리뷰 라운드 조정
